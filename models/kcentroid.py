...
from utils.metrics import inertia # 클러스터 개수인 K값을 결정할 때 참고할 수 있는 지표로, k값에 따른 inertia 값 변화율 추이를 보고 효과적인 k값을 결정 할 수 있다.
import argparse
import time
import matplotlib.pyplot as plt
import matplotlib.cm as cm
from sklearn.metrics import silhouette_samples, silhouette_score # Clustering의 품질을 정량적으로 평가해주는 지표. [0-1] 1에 가까울 수록 우수한 품질 
...
class KMeans:
    ...
    def _update(self):
        ...
        클러스터의 중심과 각 클러스터에 속해 있는 데이터간의 거리를 계산하여, 그 중 가장 거리가 가까운 클러스터에 데이터를 할당한다.
    ...
    def fit(self, datas):
        """ 초기 클러스터의 중심을 랜덤하게 초기화 해주는 initialization 과정과,
        새로운 군집 변화에 대하여 클러스터의 중심을 다시 계산하는 centroid Update 과정을 수행하는 함수이다.
        centroid Update 과정으로 클러스터의 중심이 갱신된 후, update함수가 갱신된 클러스터를 중심으로 군집을 갱신한다.


        Args:
            datas (list): 원본 data중 'order date (Date Orders)', 'Order Item Quantity','Order Item Total'
